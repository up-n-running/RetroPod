...
Shadow::Shadow (float* linesX, float* linesY, int noOfLines) {
...
void Shadow::finishShadowLinkedList (Node *nThis, Node* nLast, Surface *mySurface, bool drawIt) {
  //*************DESTROY LINKED LIST********************
  ...
  while (nThis != NULL) {
    if (drawIt) 
      glVertex3f (nThis->x, mySurface->getHeightAt(nThis->x, -nThis->y)+0.05f, -nThis->y );  //GETHEIGHTAT COULD FUCK UP
  ...
  for (int row=minRow ; row <= maxRow ; row++){
    for (int col=minCol ; col <= maxCol ; col++) {
      for (int topLeft=0 ; topLeft < 2 ; topLeft++ ) {
        ...
        initShadowLinkedList(col, row, topLeft, &nThis, &nLast);
